...
SET logCache TO "".
Log2("UT;Message;Alt;Speed;VSpeed;HSpeed;Acc;GA;MAC;Throttle;SBD;TTI;SRP;DTLZ").
...
IF DEFINED TWP
{
	...
	FOR WPItem IN WPList
	{
		IF WPItem:NAME = TWP
		{
			...
			Echo("Target Waypoint (WP): " + WP:NAME + " @ " + TGEO + " (" + ROUND(TGEO:DISTANCE) + " m away)").
		}
	}
}
...
LOCK SRP TO 90 - VANG(SRFRETROGRADE:FOREVECTOR, UP:FOREVECTOR).  // Surface retrograde pitch
LOCK srfSpeed TO VELOCITY:SURFACE:MAG.  // Surface speed
LOCK GA TO BODY:MU / ((BODY:RADIUS + ALTITUDE / 2) * (BODY:RADIUS + ALTITUDE / 2)) - GROUNDSPEED * GROUNDSPEED / (BODY:RADIUS + ALTITUDE / 2).  // Gravitational acceleration at half the current altitude
...
LOCK TTI TO (SQRT(VERTICALSPEED * VERTICALSPEED + 2 * ALT:RADAR * GA) + VERTICALSPEED) / GA.  // Time to impact without thrust
...
// Suicide burn
WHEN ((TTI <= SBD) OR ((DEFINED TGEO) AND (TGEO:DISTANCE <= DTLZ))) AND (srfSpeed > maxSpeed) AND (THROTTLE = 0) THEN
{
...
UNTIL exit
{
	...
	IF DEFINED TGEO { Display("DTT:   " + ROUND(TGEO:DISTANCE) + " m"). }
	...
	IF (TIME:SECONDS >= logTime)
	{
		...
		SET lastSpeed TO srfSpeed.
		Log2(TIME:SECONDS + ";;" + ROUND(ALT:RADAR) + ";" + ROUND(srfSpeed, 1) + ";" + ROUND(VERTICALSPEED, 1) + ";" + ROUND(GROUNDSPEED, 1) + ";" + ROUND(ac, 2) + ";" + ROUND(GA, 2) + ";" + ROUND(MAC, 3) + ";" + ROUND(THROTTLE * 100) + "%;" + ROUND(SBD, 1) + ";" + ROUND(TTI, 1) + ";" + ROUND(SRP, 1) + ";" + ROUND(DTLZ)).
